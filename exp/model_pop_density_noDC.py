...
for state in np.unique(pop_data['Province_State']):
    if state == 'Grand Princess' or state == 'Diamond Princess' or state == 'District of Columbia':
        continue
    sum_pop = 0
...
#regression
p = np.poly1d(np.polyfit(densities, tests, 15))
x_range = np.linspace(0, 1150)
plt.ylim(bottom=0)
line = p(x_range)
...
features = np.ones((len(densities), 2))
for i in range(len(densities)):
    features[i, 0] = densities[i]
...
print('Given population density ' + str(input_pop_tests[0]) + ' and number tested ' + str(input_pop_tests[1]) + ', the predicted death level = ' + str(prediction_death))

#plot new pop, test, death level
plt.scatter(input_pop_tests[0], input_pop_tests[1], color=prediction_death, marker='^')


plt.savefig('model_pop_density_noDC.png')
